...
function [Params, val,nor_dis,nor_dis2,jj]=mysolve_EP_f(Conc,Time,NR,Params,DATA,MEAN_initial,num_sub,lb,ub,max_iter,optimality_tol,step_tol)
    step_size=3e-1;
    thro=step_size^2;
    low_n=0.2;
    ...
    %         
        ...
        f2= BD_Obj_full(x1,DATA,Conc,Time,MEAN_initial,'Obj'); %only function value is need
    ...
    % sigma
                ...
                f2= BD_Obj_full(x1,DATA,Conc,Time,MEAN_initial,'Obj'); %only function value is needed
            else
                break
            end
