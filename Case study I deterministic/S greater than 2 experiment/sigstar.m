function varargout=sigstar(groups,stats,nosort,scale_cmd)
    ...
    if scale_cmd == 0
        ...
        for ii=1:length(groups)
            ...
            % thisY = max(y) - (length(groups)-ii)*yd;
            % max(y)*exp(ii*0.05*max(y))
            % 10*(length(groups)-ii)*yd
            % exp((length(groups)-ii)*yd)
            % thisY = max(y)*exp((length(groups)-ii)*yd);
    ...
    if scale_cmd == 0
        ...
        yd=myRange(log10(ylim))*0.01; %Ticks are 1% of the y axis range
        for ii=1:length(groups)
            y=get(H(ii,1),'YData');
            y(1)=y(1)/(10^yd);
            y(4)=y(4)/(10^yd);   
