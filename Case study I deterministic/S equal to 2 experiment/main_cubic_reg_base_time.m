...
ub=[1;1;repmat([1;1;Inf;Inf],2,1)]; % The upper bound is seted as for [ub_a,ub_b,ub_E^n,ub_n]
...
for i = 1:num_idx 
    ...
    GE_lb = [alpha_lb,b_lb,E_s_lb,m_lb,alpha_lb,b_lb,E_r_lb,m_lb];
    GE_ub = [alpha_ub,b_ub,E_s_ub,m_ub,alpha_ub,b_ub,E_r_ub,m_ub];
    
    THETA = rand(1,8).*(GE_ub-GE_lb)+GE_lb;
    THETA = [rand*0.5,THETA,NoiseL,NoiseH];
    ...
    fmin_IP_number_init = [fmin_IP_number_init,ii];
    t_IP=toc;
    ...
    fmin_IP_hess_number_init = [fmin_IP_hess_number_init,ii];
    t_IP_hess=toc;
    ...
    fmin_sqp_number_init = [fmin_sqp_number_init,ii];
    t_sqp=toc;
    ...
    fmin_sqp_grad_number_init = [fmin_sqp_grad_number_init,ii];
    t_sqp_grad=toc;
...
%         end
        ...
        x_list=[x_list,[Pop;vec(param_)]];         % record the solution
    ...
    cubic_number_init = [cubic_number_init,ii];
    t_cubic=toc;
    ...
    for ii=1:num_opt
        ...
        Pop=param_instance(1:num_sub);
        param=reshape(param_instance(num_sub+1:end),4,[]);
        [param,param_, Pop,val,nor_dis,nor_dis2,iter]=mysolvefirst(Conc,Time,NR,Pop,param,Observations_ave,INIT,num_sub,param_acc,param__acc,lb,ub,max_iter,opt_tol,step_tol);
%         if val(end)<best
%             best=val(end);
%             val0=val;
%         end
        val_FAS_i=[val_FAS_i,val(end)];                    % record the objective
        nor_dis_FAS_i=[nor_dis_FAS_i,nor_dis(end)];
        nor_dis_FAS_i_2=[nor_dis_FAS_i_2,nor_dis2(end)];  
        ...
        x_FAS_list=[x_FAS_list,[Pop;vec(param_)]];         % record the solution
        iter_FAS = [iter_FAS,iter];            % record the iteration
        if toc > Time_tol
            break
        end
    end

    FAS_number_init = [FAS_number_init,ii];
    t_FAS=toc;
    ...
    [B_FAS,I_FAS]=mink(val_FAS_i,num_best);
    ...
    Theta_acc = [Pop_acc,reshape(param_acc,1,8)];
    
    cubic_val_hist = [cubic_val_hist,B];
    FAS_val_hist   = [FAS_val_hist,B_FAS];
    ...
    FAS_err_hist   = [FAS_err_hist,abs(x_FAS_list(:,I_FAS)-Theta_acc')];
    ...
    FAS_est_hist   = [FAS_est_hist,x_FAS_list(:,I_FAS)];
    ...
    FAS_iter_hist   = [FAS_iter_hist,iter_FAS(I_FAS)];
...
name = strcat('Result/optTime1_',num2str(num_idx),'_uc.mat');
save(name)